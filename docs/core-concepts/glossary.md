---
title: "术语表"
description: "Eve、Workflow、沙盒、技能、工具等核心术语解释。"
---
...
Vercel 推出的 filesystem-first durable AI agents 框架。它用 `agent/` 目录组织 Agent 能力，并结合 AI SDK、Workflow SDK、沙盒、渠道、技能、工具等机制。
...
agent/skills/foo/SKILL.md   技能 foo
agent/subagents/bar/        子智能体 bar
```
...
技能不增加执行能力，只给模型补充过程性说明。真正执行动作仍靠 tools。

## subagent

子智能体。用于把某类任务委派给专门的 Agent，例如风险审查、报告写作、知识库检索。
...
Workflow 层的一次运行。Eve 的 session、turn、子智能体调用背后都可能对应 workflow run。
