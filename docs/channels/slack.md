...
Message hooks 返回 `{ auth }` 以 dispatch、`null` 以丢弃，或 `{ auth, context }` 把背景信息注入 history。公开对话使用返回的 `title`，省略时用触发消息文本；**DM 与私有频道**的 run 标题固定为 `Private message`，避免标题泄露消息内容。
...
HITL 渲染为 Slack 按钮和下拉选择。固定选项与自由文本 modal 提交会先经 `onInputResponse`，再恢复 parked session。初始的 **Type your answer** 只打开 eve 的 modal；用户真正提交答案时才跑 hook。

用 `approvalChannel` 选择每条输入请求出现在哪里（含工具审批与 `ctx.ask()` 问题）：返回 `"direct-message"` 仅对触发 turn 的 Slack 用户可见，返回 `"thread"` 走普通共享线程。
...
回调收到 `InputRequest` 与当前 `SessionContext`，可按 `kind`、`action` 或展示属性切换。省略则全部留在线程；无条件返回 `"direct-message"` 则全部私聊。若无法解析触发用户做 DM，eve 只记日志、不发回退帖——私有投递 fail-closed。谁能点选仍由既有 `onInputResponse` / 工具审批策略决定。

作者自定义的 `input.requested` handler 可先补充投递，再通过 `defaultDeliver()` 把请求交回 eve 的标准线程 / 私聊投递。例如先在线程贴非敏感上下文，再让 eve 渲染可操作控件；已配置的 `approvalChannel` 仍会把敏感审查送 DM、普通请求留在线程：
