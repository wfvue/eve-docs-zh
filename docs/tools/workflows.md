...
- **`ctx.ask`**：在 session channel 上发 `input.requested`（渲染方式类似审批），返回可 await 的答案；await 会挂起 run。opt-in 的 `ask_question` 就是建立在 `ctx.ask` 上的 workflow 工具。答案 `status`：`answered`（选了选项或打了字）、`dismissed`（请求设了 `dismissible: true` 且用户发了未作答消息——仅阻塞工具）、`unavailable`（session 无法请求输入，如 scheduled run）。仅当它是唯一挂起提问时，纯文本 follow-up 才能作答；后台 / 子智能体提问需结构化响应。请求属于 run 而非 turn；结束 run（return / throw / 取消）会撤回 pending 请求。可用 `Promise.race([pending, sleep("4h")])` 加截止。
...
当父模型应决定「要不要委派」，但应由 [Jev](https://vercel.com/i/what-is-jev) 选择专家时，写一个 authored workflow 工具做路由。给每个专家设 `tool: false`，让父模型只看到路由工具；专家仍可通过 `ctx.agents` / `ctx.agent()` 调用。
...
export default defineAgent({
  ...
  description: "Research and summarize primary sources.",
  tool: false,
});
```
...
export default defineAgent({
  ...
  description: "Apply changes and run operational checklists.",
  tool: false,
});
```

在 `agent/tools/` 下定义面向模型的 router：用子智能体的有效 description 拼 Jev criteria，再把类型化 choice 直接传给 `ctx.agent()`。评估放在 `"use step"` 里，以便 workflow replay 记录结果、避免重复路由请求。父模型看到的是 router，而不是 `researcher` / `operator`。

也可用 `agentRouter()` helper：输入 `{ message, outputSchema? }`。有两个及以上带 description 的目标时，它把 message 与 description 交给 Jev，再 `ctx.agent()` 调用选中的名字；只有一个目标时跳过评估。这控制「选哪个专家」，不控制「父模型是否委派」。若每个入站请求都必须经 Jev，请在父模型跑之前路由。

更多见官方 [Workflows](https://eve.dev/docs/tools/workflows#route-to-a-hidden-subagent-with-jev) 与 [evaluate](../guides/evaluate)。
