---
title: "CLI"
description: "eve CLI 每个命令的参考：init、set、info、build、start、dev、logs、traces、link、deploy、eval、channels、extension 与 telemetry。"
---
...
| `eve telemetry <command>` | 查看、开启或关闭 CLI 遥测 |

`eve build` 因发现错误失败时，打印完整诊断与 diagnostics artifact 路径。

## CLI 遥测

eve 默认收集 CLI 遥测以改进命令行体验。对本机关闭：`eve telemetry disable`；只对单次命令关闭：设 `EVE_TELEMETRY_DISABLED=1`。当前字段、排除项、debug 模式、提示文案与本机偏好存储见 [CLI 遥测](./telemetry)。
