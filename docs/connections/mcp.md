...
## 配置协议发现（Configure protocol discovery）

**官方说明：** eve 默认会发现远端 MCP 协议版本。若某台服务器只支持较旧的 `initialize` 握手，可按连接关闭发现：
...
设为 `false` 时，客户端跳过 `server/discover`，直接以 `initialize` 开始，提出 `2025-11-25` 并协商握手可用的版本。这是**按连接**的开关：省略或设为 `true` 保持发现开启。服务器已支持发现时去掉该覆盖。动态 resolver 返回的连接同样适用。
