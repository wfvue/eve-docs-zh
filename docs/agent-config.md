...
直接访问 OpenAI 或 Anthropic 时，可用 eve 自带 helper，不必再装其它 provider 包：
...
eve 在等待输入 / 授权 / 阻塞协调，或确认后台任务之前会结束当前 batch；turn 结束或 steering 到达时，也可能低于上限提前收束。助手输出开始前，steering 可以打断待进行的模型生成；正在执行的 tools 会安全收尾后，批次再让出并应用校正。此选项 experimental，任意版本可能变更或移除。重试语义见 [执行模型与持久性](./concepts/execution-model-and-durability)。
