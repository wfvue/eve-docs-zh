...
- 抛错或无效的 `recall["turn.started"]` 会在模型调用前失败整个 turn；任何 slot 的召回结果都不会提交。若 turn 的 `abortSignal` **已经** aborted，eve 把该错误当作取消，并继续处理排队中的后续消息。信号仍活跃时的 `AbortError` 仍会让 turn 失败。
