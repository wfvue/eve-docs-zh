...
- **`meta.deliveryIds`**（若存在）标识拥有该 turn 的已接受消息。`POST /eve/v1/session/:sessionId` 会返回其 `deliveryId`，客户端从旧 cursor 恢复时可跳过更早的 turn。合并投递的消息共享同一组 turn 事件。
...
阻塞工具里挂起的 `ctx.ask()` 提问（例如 opt-in 的 `ask_question`）可用纯文本作答：当它是**唯一**挂起提问时，匹配选项的消息会回答它；若提问允许自由文本，任意消息也可作答。否则 eve 不会猜测文本寻址哪个提问：消息按正常 `turnPolicy` 走，`dismissible: true` 的提问解析为 `dismissed`，其它提问保持打开。后台工具与子智能体的提问始终需要结构化响应。用结构化响应无歧义地定向请求。
...
默认情况下，已 admit 的后台任务在发起 turn 取消后仍存活。传 `tasks: true` 可一并取消该 session 拥有的全部后台任务；session 处于 park 时也可用，从而无需 reset 就能停掉后台工作：
...
`"accepted"` 表示活跃 session 已 durable 排队该请求；取消异步完成。在流上以 `turn.cancelled` 后跟 `session.waiting` 确认 turn 取消；任务是否取消请在后续 turn 查看 task 状态。session 随后正常接受下一条消息。尚未 admit 的后台工作随取消的 step 被拒绝。每个被取消的 child 在自己的 child-session stream 上报告边界。活跃但已 park 的 session 返回 `"accepted"`：普通取消是 no-op，而 `tasks: true` 仍会取消已索引任务。`"no_active_turn"` 表示 session 或 channel 地址未知或已终结。两种状态都是成功，所以客户端可以 fire and forget。完整路由契约见 [eve channel](../channels/eve)。
