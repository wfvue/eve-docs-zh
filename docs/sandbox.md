...
默认情况下，Docker 与 microsandbox 拉取 `ghcr.io/vercel/eve`，Vercel Sandbox 拉取 `vcr.vercel.com/vercel/eve/base`；镜像 tag 与已安装的 eve 版本一致，但**不含** build metadata（例如 commit tarball `0.56.0+git.4c27b3e…` 使用 tag `0.56.0`）。可用 `EVE_SANDBOX_IMAGE_TAG` 覆盖这些默认镜像的版本派生 tag。传给 `docker()`、`microsandbox()` 或 `vercel()` 的显式 `image` 优先。`vercel()` 上的 snapshot `source` 又优先于其 `image`，因为 Vercel Sandbox 把二者视为互斥。
