...
Web Chat 会安装项目级 Next.js 应用。它**不能**加到 eve agent workspace；eve 会在写文件前拒绝安装。浏览器应用放哪见[项目结构](./concepts/project-structure)，手动集成见 [Next.js](./guides/frontend/nextjs)。
...
使用 registry 发布者提供的 integration URL 模板，并给这个源一个命名空间：
